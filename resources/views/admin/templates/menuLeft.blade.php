<div id="left-menu">
    <div class="left-menu-title">

    </div>
    <ul>
        <p class="left-menu-main">Sản phẩm</p>
        <li>
            <a>
                <svg class="bi bi-bucket-fill" width="1em" height="1em" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" d="M8 1.5A4.5 4.5 0 003.5 6h-1a5.5 5.5 0 1111 0h-1A4.5 4.5 0 008 1.5z" clip-rule="evenodd"/>
                    <path fill-rule="evenodd" d="M1.61 5.687A.5.5 0 012 5.5h12a.5.5 0 01.488.608l-1.826 8.217a1.5 1.5 0 01-1.464 1.175H4.802a1.5 1.5 0 01-1.464-1.175L1.512 6.108a.5.5 0 01.098-.42z" clip-rule="evenodd"/>
                </svg>
                <span>
                    Category
                </span>
                <i class="fa fa-chevron-down icon-menu-close icon-down" aria-hidden="true"></i>
                <i class="fa fa-chevron-up icon-menu-close icon-up" aria-hidden="true"></i>
            </a>
            <div class="menu-child">
                <div class="menu-child-item">
                    <a href="{{ url('/admin/category/add') }}">Thêm mới</a>
                </div>
                <div class="menu-child-item">
                    <a href="{{ url('/admin/category/list') }}">Danh sách</a>
                </div>
            </div>
        </li>
        <p class="left-menu-main">Dịch vụ</p>
        <li>
            <a>
                <svg class="bi bi-bucket-fill" width="1em" height="1em" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" d="M8 1.5A4.5 4.5 0 003.5 6h-1a5.5 5.5 0 1111 0h-1A4.5 4.5 0 008 1.5z" clip-rule="evenodd"/>
                    <path fill-rule="evenodd" d="M1.61 5.687A.5.5 0 012 5.5h12a.5.5 0 01.488.608l-1.826 8.217a1.5 1.5 0 01-1.464 1.175H4.802a1.5 1.5 0 01-1.464-1.175L1.512 6.108a.5.5 0 01.098-.42z" clip-rule="evenodd"/>
                </svg>
                <span>
                    Category
                </span>
            </a>
            <div class="menu-child">
                <div class="menu-child-item">
                    <a>Thêm mới</a>
                </div>
                <div class="menu-child-item">
                    <a>Danh sách</a>
                </div>
            </div>
        </li>
        <li>
            <a href="">
                <svg class="bi bi-bucket-fill" width="1em" height="1em" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" d="M8 1.5A4.5 4.5 0 003.5 6h-1a5.5 5.5 0 1111 0h-1A4.5 4.5 0 008 1.5z" clip-rule="evenodd"/>
                    <path fill-rule="evenodd" d="M1.61 5.687A.5.5 0 012 5.5h12a.5.5 0 01.488.608l-1.826 8.217a1.5 1.5 0 01-1.464 1.175H4.802a1.5 1.5 0 01-1.464-1.175L1.512 6.108a.5.5 0 01.098-.42z" clip-rule="evenodd"/>
                </svg>
                <span>
                    Sản phẩm
                </span>
            </a>
        </li>
    </ul>
    <div id="copy-right">
        CoreUI © 2020 creativeLabs.
    </div>
</div>
